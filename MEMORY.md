...
| 5 | Язык | Только RU |
| 6 | Оценка качества лидов | Маппинг на Next.js API (100 -> 5 звезд, 50 -> 3, 10 -> 1) с выводом ИИ-чеклиста и 🔥/⚡/💤 бэйджей в CRM |
| 7 | Сценарий захода в CRM | Отображается в виде превью-блока с кнопкой быстрого копирования и переключением в режим редактирования |
...
### ЭТАП 4 ✅ N8n интеграция и CRM-панель (2026-07-02)
...
- [x] Contact form webhook с валидацией.
- [x] Интеграция API CRM-панели (`/api/leads`): получение полного списка лидов (через n8n `responseNode` + массив), обновление статусов, создание лидов.
- [x] Интеграция ИИ-диагностики лидов: промпт-фильтрация отзывов в n8n на предмет коммуникационных болей, расчет ИИ-рейтинга качества (звезды 1/3/5), сохранение чеклиста в `data_quality` в Postgres и его отображение на фронтенде.
- [x] Повышение читаемости сценария захода: превью-блок с кнопкой быстрого копирования и увеличенная зона редактирования (250px).
